...
(function (layui, window, factory) {
	...
	var v = '4.0.0.0910',
		...
		SHOW_HEIGHT = 'xm-select-showheight',
	    ...
	    quickBtns = [{ icon: 'xm-iconfont icon-quanxuan', name: 'すべて選択', click: function click(id, cm) {
			cm.selectAll(id, true, true);
		} }, { icon: 'xm-iconfont icon-qingkong', name: 'クリアー', click: function click(id, cm) {
			cm.removeAll(id, true, true);
		} }, { icon: 'xm-iconfont icon-fanxuan', name: '反転選択', click: function click(id, cm) {
			cm.reverse(id, true, true);
		} }, { icon: 'xm-iconfont icon-pifu', name: 'スタイル', click: function click(id, cm) {
			cm.skin(id);
		} }],
	...
	Common.prototype.init = function (target) {
		...
		$(target ? target : 'select[' + NAME + ']').each(function (index, select) {
			var othis = $(select),
				...
				showheight: othis.attr(SHOW_HEIGHT),
			...
			var showHeight  = !fs.config.showheight || fs.config.showheight == 'auto' ? '' : 'max-height: '+fs.config.showheight+'px;';
	...
	Common.prototype.search = function (id, e, searchUrl, call) {
		...
		if (searchUrl) {
			...
			if (!shows.length) {
				reElem.find('dd.' + FORM_NONE).addClass(FORM_EMPTY).text('オプションが見つかりませんでした');
			} else {
