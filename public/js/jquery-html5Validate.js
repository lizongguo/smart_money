(function($, undefined) {
	...
	OBJREG = {
		...
		"prompt": {
			...
			checkbox: "ご選択ください",
			select: "ご選択ください",
            email: "正しいメールアドレスをご記入ください。",
            url: "正しいURLをご記入してください。",
            tel: "正しい電話番号をご記入ください。",
			number: "正しい数字をご記入ください。",
			date: "正しい日付をご記入ください。",
			pattern: "フォーマットが正しく有りません。",
			empty: "请输入",
			multiple: "複数のデータはカンマ区切りを使用します。"
		}	
	};
			...
			isOverflow: function(ele) {
				...
				if (!attrMin && !attrMax) {
					...
					if (attrDataMin && value.length < attrDataMin) {
						$(ele).testRemind(attrDataMin + "文字以内ご記入ください。");
						ele.focus();
					} else if (attrDataMax && value.length > attrDataMax) {
						var name = $(ele).attr('name');
						if (name == 'emergency_postal_code' || name == 'postal_code') {
							$(ele).testRemind("半角数字" + attrDataMax + "桁でご記入ください。");
						} else if (name == 'emergency_cell_phone' || name == 'cell_phone' || name == 'out_cell_phone') {
							$(ele).testRemind(attrDataMax + "文字以内ご記入ください。");
						} else {
							$(ele).testRemind(attrDataMax + "文字以内ご記入ください。");
						}
					...
					if (attrMin && value < attrMin) {
						$(ele).testRemind(attrMin +"より大きい数字をご記入ください");
					} else if (attrMax && value > attrMax) {
						$(ele).testRemind(attrMax +"より小さい数字をご記入ください");
					} else if (attrStep && !/^\d+$/.test(Math.abs((value - attrMin || 0)) / attrStep)) {
						$(ele).testRemind("正しい数字をご入力ください");
					} else {
						return false;
					}
					ele.focus();
				  ...
				  , remind = function(control, type, tag) {
					...
					if ($(control).isVisible()) {
						if (type == "radio" || type == "checkbox") {
							...
							// 下拉值为空或文本框文本域等为空
							$(control).testRemind((tag == "empty" && text)? "まだ記入していません"+ text : OBJREG.prompt[tag]);
							control.focus();
						} else if (/^range|number$/i.test(type) && Number(control.value)) {
							// 整数值与数值的特殊提示
							$(control).testRemind("正しい数字をご記入ください");
							control.focus();
							control.select();
						} else {
							// 文本框文本域格式不准确
							...
							if (text) {
								finalText = text +"のフォーマットが正しく有りません。";
							}
						...
						var customTxt = ((!/^radio|checkbox$/i.test(type) && text) || "当該項目") + "はまだ" + (key || (tag == "empty"? "記入": "選択")) + "していません。";
						if (target.size()) {
	...
	$.fn.extend({
		...
		html5Validate: function(callback, options) {
			...
			$(this).bind("submit", function() {
				if ($.html5Validate.isAllpass($(this).find(":input"), params) && params.validate() && $.isFunction(callback)) {
					callback.call(this);	
				}
				return false;	
			});
