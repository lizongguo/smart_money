    ...
    <div class="layui-card">
        ...
        <div class="layui-card-body">
            ...
            <script type="text/html" id="stateTpl">
                @{{#  if(d.status == 1){ }}
                <button class="layui-btn layui-btn-xs">公開する</button>
                @{{#  } else { }}
                <button class="layui-btn layui-btn-danger layui-btn-xs">無効化</button>
                @{{#  } }}
            </script>

            <script type="text/html" id="titleTpl">
                <a target="_blank" class="layadmin-backlog-body" href="@{{  d.url }}"> @{{  d.title }} </a>
            </script>
...
layui.config({
    ...
    table.render({
        ...
        , url: '{{route("popular.items")}}' //模拟接口
        , cols: [[
                // {type: 'checkbox', fixed: 'left'}
                {field: 'id', width: 100, title: 'ID', sort: true}
                , {field: 'title', title: 'タイトル', sort: true, templet: '#titleTpl'}
                , {field: 'sort', title: 'ソート', width: 120, sort: true}
                , {field: 'status', title: '状態', width: 120, sort: true, templet: '#stateTpl'}
    ...
    var deleteUri = "{{route('popular.delete', ['id' => '_id'])}}";
    var editUri = "{{route('popular.input', ['id' => '_id'])}}";
...
//                        iframeWindow.layui.form.render();       
                ...
                iframeWindow.layui.form.on('submit(' + submitID + ')', function (data) {
                    ...
                    if (typeof field['data[status]'] == 'undefined') {
                        field['data[status]'] = 0;
                    }
